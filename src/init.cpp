...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the WELLand zWELLmoney supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in WELL/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in WELL/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in WELL/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zcrstake=<n>", strprintf(_("Enable or disable staking functionality for WELLinputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zzcrstake=<n>", strprintf(_("Enable or disable staking functionality for zWELLinputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzzcr=<n>", strprintf(_("Enable automatic wallet backups triggered after each zWELLminting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zzcrbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zWELLbackup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizezcoreamount=<n>     " + strprintf(_("Keep N WELLanonymized (default: %u)"), 0) + "\n";
        ...
        do {
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZWELLMinted();
                        RecalculateZWELLSpent();
                    }
                    RecalculateWELLSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzWELLWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zWELLWallet
        uiInterface.InitMessage(_("Syncing zWELLwallet..."));
...
//    if (nAnonymizeZCoreAmount < 2) nAnonymizeZCoreAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1WELL+1000 == (.1WELL+100)*10
       10WELL+10000 == (1WELL+1000)*10
    */
