...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered zcore address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 WELL.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal zcore:URI (zcore:XyZ...?message=example)
            {
...
static void NotifyzWELLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzWELLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzWELLReset.connect(boost::bind(NotifyzWELLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzWELLReset.disconnect(boost::bind(NotifyzWELLReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zzcrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
