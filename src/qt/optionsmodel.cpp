...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::WELL);
    ...
    if (!settings.contains("nAnonymizeZCoreAmount"))
        settings.setValue("nAnonymizeZCoreAmount", 1000);

    nAnonymizeZCoreAmount = settings.value("nAnonymizeZCoreAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZCoreAmount"))
        SoftSetArg("-anonymizezcoreamount", settings.value("nAnonymizeZCoreAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zcore.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZCoreAmount:
            return QVariant(nAnonymizeZCoreAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZCoreAmount:
            nAnonymizeZCoreAmount = value.toInt();
            settings.setValue("nAnonymizeZCoreAmount", nAnonymizeZCoreAmount);
            emit anonymizeZCoreAmountChanged(nAnonymizeZCoreAmount);
            break;
        case CoinControlFeatures:
