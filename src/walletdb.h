...
public:
    ...
    bool WriteZWELLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZWELLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZWELLSeed_deprecated(uint256& seed);
    bool EraseZWELLSeed();
    bool EraseZWELLSeed_deprecated();

    bool WriteZWELLCount(const uint32_t& nCount);
    bool ReadZWELLCount(uint32_t& nCount);
