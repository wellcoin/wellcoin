...
CzWELLWallet::CzWELLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zzcr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWELLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWELLSeed_deprecated()) {
                    LogPrintf("%s: Updated zWELLseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zzcr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zzcr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzWELLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWELLCount(nCountLastUsed);
    else if (!walletdb.ReadZWELLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWELLWallet::Lock()
{
    seedMaster = 0;
}

void CzWELLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWELLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWELL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWELLWallet::LoadMintPoolFromDB()
{
...
void CzWELLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWELLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWELLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zzcrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzWELLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWELL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zzcrTracker which also adds to database
    pwalletMain->zzcrTracker->Add(dMint, true);
...
void CzWELLWallet::SeedToZWELL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWELLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWELLWallet::GenerateDeterministicZWELL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWELLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWELLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
